...
GCC_BIN:=C:\nxp\LPCXpresso_4.2.2_275\lpcxpresso\Tools\bin\arm-none-eabi-gcc.exe
GXX_BIN:=C:\nxp\LPCXpresso_4.2.2_275\lpcxpresso\Tools\bin\arm-none-eabi-g++.exe
...
ECHO:=echo
TOUCH:=touch
AWK:=C:\nxp\LPCXpresso_4.2.2_275\lpcxpresso\msys\bin\gawk.exe
...
# 	    S && /Library\/Frameworks/ {next;}		\	    
	$(GXX) -v -c $(E).cpp 2>&1 | $(AWK) -f awk-program >gcc-include-path.lnt
...
#NOTE: The previous does not work. You need to take the resulting
#co-gcc.mak.temp-generate-size-options.cc and compile it under code_red
#or whatever, print the results to serial port or some other debug area,
#and shove them in size-options.lnt
